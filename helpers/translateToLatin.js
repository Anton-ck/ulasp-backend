function translit(str) {
  ...
  let res = '';
  for (let i = 0, l = str.length; i < l; i += i) {
...
export default function generateLatinTranslation(str) {
  const fileName = str.replace(/[\s]+/gi, '-');
  const filenameFinall = translit(fileName);
